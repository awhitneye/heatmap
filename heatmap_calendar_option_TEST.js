// Optional calendar heatmap that, before inclusion to dashboard, allows for selection of
// color range based on data range, or a fixed range.
function calendarHeatmap() {
	...
	chart.scaleRange = function (value) {
		if (!arguments.length) {
			return scaleRange;
		}
		scaleRange = value;
	...
	function chart() {
		...
		// fixes the data range correlation to color range:
		if (chart.data().length == 0) {
			max = 0;
		} else if (max === null) {
			// assigns max data value for the first time
			max = d3.max(chart.data(), function (d) {
				return d.count;
			});
			if (chart.scaleRange() === 'Data') {  // if based on the range of the data (not a fixed range)
			...
			} else {  // based on a fixed range.  If calculated max is over 2, create an overflow.
				if (max < 2) {
					max = 2;
				}
			}
		...
		function drawChart() {
			...
			if (chart.legendEnabled()) {
				var colorScale = [color(min), color(q1), color(q2), color(q3), color(max)];
				...
				legendGroup.append('text')
					...
					.attr('x', (width - legendWidth + SQUARE_PADDING) + (colorScale.length + 1) * SQUARE_LENGTH)
					.attr('y', height + Math.floor(SQUARE_LENGTH * 3 / 4))
					.text(locale.More);
			}
...
looker.plugins.visualizations.add({
	...
	options: {
		...
		fixMin: {
			type: "string",
			label: "Set Min Data Val for Color Range",
			default: 0,
			hidden: function (config, queryResponse) {
				return config.colorScaleTypeToggle === "Data";
			}(),
			order: 2
		},
		colorQ1: {
		...
		fixQ1: {
			type: "string",
			label: "Set Q1 Data Val for Color Range",
			default: .5,
			hidden: function (config, queryResponse) {
				return config.colorScaleTypeToggle === "Data";
			}(),
		...
		fixQ2: {
			type: "string",
			label: "Set Q2 Data Val for Color Range",
			default: 1,
			hidden: function (config, queryResponse) {
				return config.colorScaleTypeToggle === "Data";
			}(),
		...
		fixQ3: {
			type: "string",
			label: "Set Q3 Data Val for Color Range",
			default: 1.5,
			hidden: function (config, queryResponse) {
				return config.colorScaleTypeToggle === "Data";
			}(),
		...
		fixMax: {
			type: "string",
			label: "Set Max Data Val for Color Range",
			default: 2,
			hidden: function (config, queryResponse) {
				return config.colorScaleTypeToggle === "Data";
			}(),
		...
		colorScaleTypeToggle: {
			type: "string",
			label: "Select Type of Range for Color Scale",
			display: "radio",
			default: "Text",
			values: [
				{"Data Range": "Data"},
	...
	updateAsync: function (data, element, config, queryResponse, details, done) {
		...
		var heatmap = calendarHeatmap()
			...
			.selector('.container')
			.tooltipEnabled(true)
			.colorScale([config.colorMin, config.colorQ1, config.colorQ2, config.colorQ3, config.colorMax])
			.scaleRange(config.colorScaleTypeToggle)
