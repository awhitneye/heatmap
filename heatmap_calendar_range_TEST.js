// Optional calendar heatmap that, before inclusion to dashboard,
// fixes color range to be based on a fixed range of percentages.
function calendarHeatmap() {
	...
	function chart() {
		...
		if (chart.data().length == 0) {
			...
			largest = d3.max(chart.data(), function (d) {
				return d.count;
			}); // largest data value
			if (largest > 2) {
				max = largest;
